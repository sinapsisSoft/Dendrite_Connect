...
class DaoClient
{
    ...
    public function newClient($objClient, $objUser)
    {
        try {
            $con = $this->objConntion->connect();
            $con->query("SET NAMES 'utf8'");
            if ($con != null) {
                $dataClient = $objClient->__getId().",'" . $objClient->__getName() . "','" . $objClient->__getIdentification() . "','" . $objClient->__getAddress() . "','" . $objClient->__getTel() . "','" . $objClient->__getEmail() . "','" . $objClient->__getContactName() . "','" . $objClient->__getContactTitle() . "','" . $objClient->__getContactTel() . "','" . $objClient->__getContactCel() . "','" . $objClient->__getContactEmail() . "'," . $objClient->__getStatId() . "," . $objClient->__getUser();
  ...
  #Description: Function list clients
  #Date:25/09/2020
  ...
  public function selectListClient()
  {
    try {
      ...
      $con->query("SET NAMES 'utf8'");
      if ($con != null) {
        if ($result = $con->query("CALL sp_select_client()")) {
          while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
            $this->arrayResult[] = $row;
          };
          mysqli_free_result($result);
